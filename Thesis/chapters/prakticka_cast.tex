...
Diagram aktivit na obrázku 3.2 zachycuje průběh práce se systémem z pohledu dvou typů aktérů, kterými jsou správce a běžný uživatel.

Správce systému zahajuje svou činnost otevřením webové aplikace, kde vytváří nové digitální dvojče. Vyplňuje formulář s potřebnými metadaty projektu a následně kontroluje správnost zadaných údajů. Pokud zjistí nesrovnalosti, má možnost data opakovaně upravovat, dokud není s výsledkem spokojen.

Běžný uživatel interaguje se systémem prostřednictvím mobilní aplikace. Procházením mapového rozhraní vybírá jednotlivé projekty podle jejich geografické polohy. Pro vybrané projekty si může prohlížet podrobné informace a vizualizovat jejich 3D modely přímo v rozšířené realitě. Po prohlédnutí má možnost projekt ohodnotit prostřednictvím hvězdičkového hodnocení a textového komentáře. Tento proces může opakovat pro libovolný počet projektů.

Správce má následně přístup k vyhodnocení uživatelských hodnocení, která může analyzovat a využít pro další rozhodování o projektech.
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.6]{images/uml5.png}
    \caption{Diagram aktivit popisující kompletní uživatelský scénář}
    \label{fig:uml5}
\end{figure}
...
Model \textit{mapsCache} slouží k optimalizaci načítání mapových podkladů. Ukládá stažené mapové dlaždice z~Google Maps API společně s~jejich parametry, kterými jsou zeměpisná šířka, zeměpisná délka, úroveň přiblížení a~časové razítko expirace. Při dotazu na mapovou dlaždici server nejprve ověří, zda existuje platná dočasně uložená verze. Pokud ano, vrátí ji přímo z~databáze, čímž se eliminují opakované dotazy na externí API a~výrazně se zrychlí odezva aplikace.
\begin{itemize}
...
Základ klientské části tvoří herní engine Unity, který je v~současnosti jedním z~nejpoužívanějších nástrojů pro vývoj AR a~VR aplikací. Hlavní výhodou Unity je jeho multiplatformnost, podpora velkého množství XR zařízení a~stabilní ekosystém knihoven. \cite{unity_manual}
